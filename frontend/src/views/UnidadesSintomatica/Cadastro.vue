<template>
  <div>
    <v-container>
      <v-card>
        <v-card-title class="headline">Cadastro das Unidades Sintomáticas</v-card-title>
        <v-card-text>
          <v-form>
            <v-row>
              <v-col cols="12" md="12">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async findById(id) {
        const response = await this.axios.get(`unidades_sintomaticas/${id}`)
        .catch(err => {
          console.log(err);
          this.$swal(
            'Erro',
